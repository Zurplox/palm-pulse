...
MAX_AI = int(os.getenv("MAX_AI_SUMMARIES", "12"))
...
def apply_summaries(stories: list[dict]) -> list[str]:
    ...
    for story in stories[:MAX_AI]:
        try:
            ...
            errors.append(f"Gemini · {story['title'][:50]}: {exc}")
            # Stop quickly on quota/auth errors; all remaining items keep extracts.
            if "429" in str(exc) or "403" in str(exc) or "400" in str(exc): break
    return errors
...
def main() -> int:
    ...
    payload = {
        ...
        "health": {"feed_errors": feed_errors, "summary_errors": ai_errors}
    }
    if existing.exists():
        try:
