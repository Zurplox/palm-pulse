...
def parse_id_number(raw: str) -> float | None:
    """Parse Indonesian-formatted numbers such as 3.890,97."""
    raw = raw.strip().replace(" ", "")
    if "," in raw:
        raw = raw.replace(".", "").replace(",", ".")
    elif raw.count(".") and all(len(p) == 3 for p in raw.split(".")[1:]):
        raw = raw.replace(".", "")
...
def parse_tbs_period(text: str) -> tuple[str, str] | None:
    ...
    for index, pattern in enumerate(patterns):
        ...
        try:
            ...
            d1, m1, y1, d2, m2, y2 = int(match[1]), MONTHS_ID[match[2].lower()], int(match[3]), int(match[4]), MONTHS_ID[match[5].lower()], int(match[6])
...
def parse_tbs_candidate(title: str, body: str, url: str, publisher: str, published: datetime) -> dict | None:
    ...
    # Prefer the standard nine-year benchmark and reject small "price increased by" values.
    targeted = re.search(r"(?:umur|usia)\s*9\s*tahun.{0,260}?(?:menjadi|sebesar|mencapai|dipatok|harga)\s*(?:rp\.?\s*)?([\d.]+(?:,\d+)?)", text, re.I)
    price = parse_id_number(targeted[1]) if targeted else None
    if price is None or not 2000 <= price <= 8000:
        values = [parse_id_number(v) for v in re.findall(r"Rp\.?\s*([\d.]+(?:,\d+)?)", text, re.I)]
        values = [v for v in values if v is not None and 2000 <= v <= 8000]
    ...
    if change_match:
        ...
        if magnitude is not None and magnitude < 1000:
            change = round(magnitude if change_match[1].lower() == "naik" else -magnitude, 2)
    ...
    official_terms = ("disbun", "riau.go.id", "siak.go.id", "media center riau", "pemprov riau", "diskominfo siak")
    priority = 0 if any(x in source_text for x in official_terms) else 1 if "infosawit" in source_text else 2
    return {
        "region": region, "reference_for": None if region == "Siak" else "Siak",
        "scheme": scheme, "palm_age_years": 9, "price_rp_per_kg": round(price, 2),
        "change_rp_per_kg": change,
...
def previous_tbs_prices() -> list[dict]:
    """Use the deployed edition first because generated JSON is not committed to main."""
    for mode in ("live", "local"):
        try:
            if mode == "live":
                response = requests.get(LIVE_JSON, params={"t": int(time.time())}, timeout=12,
                                        headers={"Cache-Control":"no-cache", "User-Agent":USER_AGENT})
                response.raise_for_status(); payload = response.json()
            else:
                payload = json.loads((DATA / "latest.json").read_text(encoding="utf-8"))
...
def collect_tbs_prices() -> tuple[list[dict], list[str]]:
    ...
    for feed_name, feed_url in TBS_FEEDS:
        try:
            response = requests.get(feed_url, timeout=22, headers={"User-Agent":USER_AGENT})
            response.raise_for_status(); feed = feedparser.parse(response.content.lstrip())
            for entry in feed.entries[:30]:
                title = clean_text(entry.get("title")); url = entry.get("link", "").strip()
    ...
    for scheme in ("Plasma", "Swadaya", "Umum"):
        ...
        if pool:
            best = pool[0]
            matching = {x["source_name"] for x in pool if x["valid_to"] == best["valid_to"] and abs(x["price_rp_per_kg"] - best["price_rp_per_kg"]) < 2}
            best["cross_checked_sources"] = len(matching)
            best["confidence"] = "official" if best["source_priority"] == 0 else "corroborated" if len(matching) > 1 else "reported"
            chosen.append(best)

    siak = sorted([x for x in candidates if x["region"] == "Siak"], key=lambda x: (x["valid_to"], x["published_at"]), reverse=True)
    if siak and all(x["region"] != "Siak" for x in chosen): chosen.insert(0, siak[0])
    ...
    for item in chosen:
        try:
            ...
            item["status"] = "current_period" if start <= today <= end else "latest_available"
        except Exception: item["status"] = "latest_available"
        change = item.get("change_rp_per_kg")
        item["trend"] = "up" if change and change > 0 else "down" if change and change < 0 else "flat"
        if change and item.get("price_rp_per_kg"):
            previous = item["price_rp_per_kg"] - change
            item["change_percent"] = round(change / previous * 100, 2) if previous else None
