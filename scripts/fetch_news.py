...
def call_gemini(model: str, prompt: str, max_tokens: int) -> str:
    endpoint = "https://generativelanguage.googleapis.com/v1beta/models/" + quote(model, safe="") + ":generateContent?key=" + quote(KEY, safe="")
    payload = {"contents":[{"parts":[{"text":prompt}]}],"generationConfig":{"temperature":0.15,"maxOutputTokens":max_tokens}}
    response = None
...
def fallback_master(stories: list[dict]) -> str:
    ...
    headlines = "; ".join(s["title"] for s in stories[:5])
    return f"Today's edition contains {len(stories)} palm-oil stories: {counts['Indonesia']} from Indonesia, {counts['Malaysia']} from Malaysia and {counts['Global']} global items. Leading headlines include {headlines}. AI synthesis was unavailable, so verify the individual publisher previews and original sources below."
...
def build_master_summary(stories: list[dict]) -> tuple[str, str, list[str], str | None]:
    ...
    prompt = f"""Create a comprehensive morning master brief from the palm-oil news items below. Synthesize the major themes, Indonesia and Malaysia developments, CPO price drivers, policy changes, risks, and likely smallholder implications. Prioritize accuracy and explicitly distinguish confirmed facts from outlook or opinion. Do not invent facts. Use clear paragraphs; there is no sentence-count limit.\n\n{source_text}"""
    errors = []
    for model in dict.fromkeys([MODEL, FALLBACK_MODEL]):
        if not model: continue
        try:
            text = call_gemini(model, prompt, 700)
            if len(text) >= 80:
...
def apply_summaries(stories: list[dict], active_model: str | None) -> list[str]:
    ...
    for story in stories[:MAX_AI]:
        try:
            prompt = f"""Summarize this palm-oil news item in 3 to 5 factual sentences, maximum 130 words. Cover what happened, the relevant numbers or policy details, and why it may matter. Use only the supplied text; do not invent facts. If the preview is too limited, say what is known without padding.\n\nHeadline: {story['title']}\nPublisher preview: {story['snippet']}"""
            summary = call_gemini(active_model, prompt, 280)
            if summary and len(summary) >= 30:
                story["summary"] = summary
                story["summary_type"] = "ai"
                story["summary_model"] = active_model
            time.sleep(12)
        except Exception as exc:
            errors.append(safe_ai_error(exc, f"Story summary ({active_model})"))
