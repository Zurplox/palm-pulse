...
# Freshness monitor. collect() already drops anything published before the
# MAX_NEWS_AGE_DAYS window, so a stale item reaching here means a feed carried a
# wrong date or the window was widened. That is worse data, not a crash, so it
# warns instead of discarding an otherwise good edition.
max_age_days = max(1, int(os.getenv('MAX_NEWS_AGE_DAYS', '4')))
for i, s in enumerate(data['stories']):
    raw = str(s['published_at']).replace('Z', '+00:00')
    try:
        published = datetime.fromisoformat(raw)
    except ValueError:
        warnings.append(f'story {i} published_at={raw!r} is not ISO-8601')
        continue
    if published.tzinfo is None:
        published = published.replace(tzinfo=timezone.utc)
    age_days = (datetime.now(timezone.utc) - published).total_seconds() / 86400
    if age_days > max_age_days + 0.5:
        warnings.append(f'story {i} is {age_days:.1f} days old, outside the '
                        f'{max_age_days}-day window: {str(s["title"])[:60]}')
for i, p in enumerate(data['tbs_prices']):
    ...
    # Warning only: every TbsPrice field is nullable in the Android app, and
    # TbsCalculations already falls back through ages 5, 6, 4, 9. A thin age
    # table is worse data, not a crash, so it must not block publishing.
    if p.get('data_quality','full_age_table') == 'full_age_table':
        for age in ('4','5','6'):
            if not float(ages.get(age,0)) > 0:
                warnings.append(f'TBS price {i} ({p.get("scheme")} {p.get("region")}) has no age {age} value')
...
# This is a WARNING, not a failure. The generator has always emitted
# datetime.isoformat() (e.g. 2026-07-26T15:28:25.358377+00:00). Moshi only needs
# a String, so the app cannot crash on it; SimpleDateFormat falls back to the
# yyyy-MM-dd pattern and reads it as midnight, which makes the freshness badge
# read STALE. That is long-standing behaviour, so it must not block publishing.
for field in ('generated_at','tbs_price_updated_at'):
    value = data.get(field)
    if value and not any(re.match(p, str(value)) for p in ts_patterns):
        warnings.append(
            f'{field}={value!r} is not an exact match for the Android date formats; '
            'the app parses the date part only, so freshness shows STALE')
