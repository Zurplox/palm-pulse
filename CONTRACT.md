...
- `stories[].published_at` must be within the freshness window
  (`MAX_NEWS_AGE_DAYS`, default 4 days). Older items are dropped by `collect()`
  before publishing; `scripts/qa.py` warns if one slips through, and never fails
  the build for it. TBS price periods are deliberately exempt, since a weekly
  price stays valid for its whole period.
