...
- Daily Riau/Siak TBS benchmark tracking with exact validity dates, source links and price trends
...
Palm Pulse checks dedicated TBS feeds every workflow run. It prioritises official Disbun/Riau/Siak publications, uses InfoSAWIT as a key fallback, and cross-checks other regional reports. It displays the nine-year plasma and swadaya benchmarks, validity period, source, change in Rp/kg and percentage trend. If no new publication is available, the latest confirmed value is retained and labelled **Latest confirmed price**. Riau values are explicitly labelled as references for Siak unless a Siak-specific publication is found. Prices are never estimated.
