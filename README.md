...
| `MAX_NEWS_AGE_DAYS` | 4 | Freshness window for the news list. Items published earlier than this are dropped at collection time, so the edition only ever shows recent news. Feed items with no usable date are treated as published now. `scripts/qa.py` warns if a story slips through older than this. |
